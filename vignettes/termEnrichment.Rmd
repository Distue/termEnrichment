...
For fast computation, the package provides parallisation using the `r Biocpkg("BiocParallel")`. If you have this package installed you can choose the *parallel = TRUE* parameter to parallise the tests.  

The results of the overrepresentation analysis can be visualized with a plotting function provided by the package. The log~2~ odds ratios are displayed of the significant terms. The signficance is displayed by the coßlours of the bars. 
...
3. optional: a vector of features for background correction (*backgroundIDs*): e.g. GeneA, GeneB, GeneC, GeneD, ..
...
```{r, eval=F}
X <- termEnrichment(termTable, foregroundIDs, backgroundIDs, annotation)
```
...
```{r, eval = F}
X <- termEnrichment(termTable, foregroundIDs, backgroundIDs, annotation, parallel = TRUE)
```
...
data("backgroundIDs", package = "termEnrichment")
```
...
The backgroundIDs would be all identified genes in the study

```{r}
head(backgroundIDs)
```

Please note, that all foregroundIDs must be contained in the backgroundID list!
...
```{r}
data("yeastInterProDesc", package = "termEnrichment")
head(yeastInterProDesc)
```
...
```{r}
GOenrichment       <- termEnrichment(yeastGO, foregroundIDs, backgroundIDs, yeastGOdesc)
InterProEnrichment <- termEnrichment(yeastInterPro, foregroundIDs, backgroundIDs, yeastInterProDesc)
```
